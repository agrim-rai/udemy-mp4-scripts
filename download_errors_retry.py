...
def load_errored_lectures(errors_log_path, csv_path):
    csv_lectures = load_lectures(csv_path)
    url_to_info = {}
    title_to_info = {}
    for row in csv_lectures:
        url = row['url'].strip()
        url_to_info[url] = row
        base_url = url.split('#')[0].rstrip('/')
        url_to_info[base_url] = row
        
        title = row['title'].strip()
        title_to_info[title] = row
        title_to_info[sanitize_filename(title)] = row

    errored = []
    if not os.path.exists(errors_log_path):
        print(f"Error: Error log file {errors_log_path} not found.")
        sys.exit(1)
    ...
    with open(errors_log_path, 'r', encoding='utf-8') as f:
        for line in f:
            ...
            if len(parts) >= 2:
                ...
                info = url_to_info.get(url_part)
                if not info:
                    base_url = url_part.split('#')[0].rstrip('/')
                    info = url_to_info.get(base_url)
                if not info:
                    info = title_to_info.get(title_part)
                
                if info:
                    if info not in errored:
                        errored.append(info)
                else:
                    print(f"Warning: Could not resolve CSV entry for '{title_part}' with URL '{url_part}'. Skipping.")
    return errored
...
def download_with_ffmpeg(m3u8_url, output_path, timeout_sec=300):
    ...
    try:
        ...
        if os.path.exists(output_path):
            try:
                ...
                pass
        raise RuntimeError(f'ffmpeg download timed out after {timeout_sec} seconds')
...
async def wait_for_m3u8(page, capture, timeout_sec):
    ...
    while time.time() < deadline:
        current_count = len(capture.urls)
        if current_count > 0 and current_count == last_count:
            await page.wait_for_timeout(1500)
            if len(capture.urls) == current_count:
                return select_best_m3u8(capture.urls)
        last_count = len(capture.urls)
...
async def capture_m3u8(page, lecture_url, timeout_sec, asset_map):
    ...
    try:
        ...
        if not m3u8_url:
            raise RuntimeError('no m3u8 stream found for this lecture')
    ...
    try:
        ...
        try:
            async with download_sem:
                await loop.run_in_executor(executor, download_with_ffmpeg, m3u8_url, output_path, download_timeout_sec)
        except Exception:
...
async def run_downloads_async(
    ...
    check_ffmpeg()
    lectures = load_errored_lectures(errors_log_path, csv_path)
    if not lectures:
        print(f"No errored lectures resolved from {errors_log_path}")
        sys.exit(0)
    ...
    error_again_path = os.path.join(output_dir, 'error_again.log')
    if os.path.exists(error_again_path):
        try:
            os.remove(error_again_path)
        except Exception:
            pass
    ...
    async with async_playwright() as p:
        ...
        print(
            ...
            f"total retry lectures: {len(lectures)}"
        )
    ...
    print(f"\nRetry Done. ok={stats.ok} skip={stats.skip} fail={stats.fail}")
    if stats.ok:
        print(f"Updated Success log: {success_log_path}")
    if stats.fail:
        print(f"Retry Error log: {error_again_path}")
...
def main():
    parser = argparse.ArgumentParser(description='Retry Udemy downloads that failed in the previous run.')
    parser.add_argument('--csv', default=DEFAULT_CSV, help='input csv path')
    parser.add_argument('--errors-log', default=os.path.join(DEFAULT_OUTPUT, 'download_errors.log'), help='input errors log file path')
    ...
    parser.add_argument('--download-timeout', type=int, default=DEFAULT_DOWNLOAD_TIMEOUT, help='seconds to wait for ffmpeg to finish downloading a video')
    parser.add_argument('--capture-workers', type=int, default=1, help='parallel browser tabs for m3u8 capture (default 1 for sequential)')
    parser.add_argument('--download-workers', type=int, default=1, help='parallel ffmpeg downloads (default 1 for sequential)')
